package com.protone.layout.view.blurView
...
abstract class BaseBlurFactory(protected val blurEngine: BlurEngine) : IBlurTool, IBlurConfig {
    ...
    protected val decorCanvas by lazy { BlurCanvas(decorBitmap!!) }
    ...
    inner class BlurCanvas(bitmap: Bitmap) : Canvas(bitmap)
}
...
class DefaultBlurController(private val root: ViewGroup, blurEngine: BlurEngine) :
    ...
    override fun blur(): Boolean {
        ...
        drawDecor()
        decorBitmap = blurEngine.blur(decorBitmap)
        return true
    }
    ...
    override fun drawBlurred(canvas: Canvas?): Boolean {
        ...
        canvas?.let {
            ...
            it.save()
            it.scale(scaleFactory.wScaled, scaleFactory.hScaled)
            decorBitmap?.let { bitmap ->
